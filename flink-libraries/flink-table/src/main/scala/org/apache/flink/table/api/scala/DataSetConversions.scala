...
import org.apache.flink.table.api.{GeneralTableEnvironment, GeneralBatchTableEnvironment, Table, TableException}
...
class DataSetConversions[T](dataSet: DataSet[T], inputType: TypeInformation[T]) {
  ...
  /**
    ...
    * the [[DataSet]].
    *
    * @param tableEnv The [[GeneralBatchTableEnvironment]] in which the new [[Table]] is created.
    * @param fields   The field names of the new [[Table]] (optional).
    * @return The resulting [[Table]].
    */
  def toTable(tableEnv: GeneralBatchTableEnvironment, fields: Expression*): Table = {
    if (fields.isEmpty) {
