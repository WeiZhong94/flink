...
class GeneralStreamTableEnvironment(env: ScalaStreamExecEnv, config: TableConfig)
  extends StreamTableEnvironment(env.getWrappedStreamExecutionEnvironment, config) {

  def this(env: JavaStreamExecEnv, config: TableConfig) {
    this(new ScalaStreamExecEnv(env), config)
  }
