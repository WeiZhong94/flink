...
import org.apache.flink.table.functions.utils.AccumulateSqlFunction.{createOperandTypeChecker, createOperandTypeInference, createReturnTypeInference}
...
class AccumulateSqlFunction(
  ...
  def getFunction: AccumulateFunction[_, _] = aggregateFunction
...
object AccumulateSqlFunction {
  ...
  def apply(
      ...
      typeFactory: FlinkTypeFactory,
      requiresOver: Boolean): AccumulateSqlFunction = {

    new AccumulateSqlFunction(
      name,
      displayName,
      aggregateFunction,
      returnType,
  ...
  private[flink] def createOperandTypeChecker(aggregateFunction: AccumulateFunction[_, _])
